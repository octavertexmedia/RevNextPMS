...
class GatewayConfigForm(forms.ModelForm):
    ...
    class Meta:
        model = GatewayConfig
        fields = ['property', 'gateway_name', 'is_active', 'api_key', 'api_secret', 'merchant_id', 'pci_compliant']
        widgets = {
            ...
            'api_key': forms.TextInput(attrs={'class': 'form-input', 'placeholder': 'API Key', 'type': 'password', 'autocomplete': 'off'}),
            'api_secret': forms.TextInput(attrs={'class': 'form-input', 'placeholder': 'API Secret', 'type': 'password', 'autocomplete': 'off'}),
